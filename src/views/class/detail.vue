      ...
      <el-form-item :label="$t('group.students')">
        ...
        <div v-if="students.length">
          <el-checkbox-group @change="checkgroupChange" v-model="checkedList">
            <el-checkbox v-for="student in students" :key="student.id" :label="student.name" checked></el-checkbox>
          </el-checkbox-group>
        </div>
      </el-form-item>
  ...
  methods: {
    ...
    checkgroupChange(v) {
      ...
      this.originStudent.forEach(student => {
        let has = false;
        for (let i = 0; i < this.checkedList.length; i += 1) {
    ...
    canEditSchool() {
      ...
      if(this.userinfo.role !== '1000') {
        this.dataModel.school = this.userinfo.school;
      } else {
        this.dataModel.school = '';
      }
