...
const UpdateStudent = async (ctx) => {
  ...
  let studentResult = await studentDao.update({
    '_id': uid
  }, {
    $set: userinfo
  });
  ...
  let userResult = await UserModel.update({
    'uid': uid
  }, {
    $set: userinfo
  });
...
const DeleteStudent = async (ctx) => {
  ...
  let studentResult = await studentDao.delete({
    '_id': userinfo.id
  });

  if (studentResult) {
    let userResult = await UserModel.remove({
      'uid': userinfo.id
    });
    if (userResult) {
      message.status = 200;
...
const GetStudentByClassId = async ctx => {
  ...
  let result = await studentDao.getByQuery({
    'group.id': userinfo.id
  });
  let count = await studentDao.countByQuery({
    'group.id': userinfo.id
  });
...
const GetCourseData = async ctx => {
  ...
  let staff = await UserModel.findOne({
    '_id': id
  });
  if (!staff) {
    message.status = 400;
  ...
  let staffObj = await StaffModel.findOne({
    '_id': staffId
  });
  if (!staff) {
    message.status = 400;
  ...
  let allStudentCourse = await CourseModel.find({
    'uid': {
      $in: studentIds
    }
  });
